...
public class Swingclient extends JFrame implements ActionListener {
	...
	public static Logger log = Logger.getLogger(Swingclient.class);
	...
	public Swingclient() {
		...
		loginButton.setBounds(315, 12, 93, 23);
		...
		register.setBounds(315, 42, 93, 23);
		getContentPane().add(register);
		
		
		// 退出
		logout = new JButton("退出");
		logout.setFont(new Font("宋体", Font.PLAIN, 12));
		logout.setActionCommand(ButtonCommand.LOGOUT);
		logout.addActionListener(this);
		logout.setBounds(315, 72, 93, 23);
	...
	public void actionPerformed(ActionEvent event) {
		...
		case ButtonCommand.LOGIN:
			try {
				...
				log.info("******登录  user:" + playerName.getText() + "  pwd: " + passward.getText());
				log.info("****** 登录   ModuleId：" + ModuleId.PLAYER + "    PlayerCmd: " + PlayerCmd.LOGIN);
		...
		case ButtonCommand.REGISTER:
			try {
				...
				log.info("******注册  user:" + playerName.getText() + "  pwd: " + passward.getText());
				log.info("****** 注册   ModuleId：" + ModuleId.PLAYER + "    PlayerCmd: " + PlayerCmd.REGISTER_AND_LOGIN);
				// 构建请求
		...
		case ButtonCommand.SEND:
			...
			break;
			// 退出
		case ButtonCommand.LOGOUT:
			try {
	
					client.getChannel().close();
					log.info("***** 退出 + client.getChannel()："+ client.getChannel());
